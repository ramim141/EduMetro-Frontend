...
const Pagination = ({
  ...
  return (
    <nav 
      className={`flex justify-center items-center space-x-1 mt-6 ${className} ${animationClasses} ${disabledClasses}`}
      ...
      {showPageNumbers && visiblePages.map((page, index) => {
        if (page === 'ellipsis-start' || page === 'ellipsis-end') {
          return (
            <span 
              key={`ellipsis-${page}`} 
              className={`${sizeClasses} flex items-center justify-center text-gray-500`}
            >
              <FaEllipsisH />
            </span>
          );
